...
namespace MathEvaluatorNetFramework.Operators
{
    internal class FactorialOperator : Operator
    {
        ...
        /// If the evaluable is a negative integer, it can raise a <see cref="DomainException"/> depending on <see cref="MathEvaluator.Parameters.RaiseDomainException"/>.<br />
        /// If the evaluable is a real floating number, it can use the Gamma function depending on <see cref="MathEvaluator.Parameters.UseGammaFunctionForNonNaturalIntegerFactorial"/>: <c>x! = gamma(x+1)</c>.
        /// </summary>
        /// <param name="variables">The used variables in the evaluable entities.</param>
        /// <returns>The factorial of the given evaluable entity. It can also return <see cref="double.NaN"/> in the case where a <see cref="DomainException"/> is canceled when <see cref="MathEvaluator.Parameters.RaiseDomainException"/> is false.</returns>
        ...
        public override double Evaluate(params Variable[] variables)
        {
            ...
            else if (evaluableResult > 0.0)
            {
                ...
                {
                    if (MathEvaluator.Parameters.UseGammaFunctionForNonNaturalIntegerFactorial)
                    {
                        result = Funcs.Gamma(evaluableResult + 1.0);
                    }
                    else if (MathEvaluator.Parameters.RaiseDomainException)
                    {
            ...
            {
                if (MathEvaluator.Parameters.UseGammaFunctionForNonNaturalIntegerFactorial && !isAnInteger)
                {
                    result = Funcs.Gamma(evaluableResult + 1.0);
                }
                else if (MathEvaluator.Parameters.RaiseDomainException)
